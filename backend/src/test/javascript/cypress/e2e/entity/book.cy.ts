...
describe('Book e2e test', () => {
  ...
  const bookSample = { isbn: 'uh-huh accomp', name: 'incidentally', publishYear: 'unknown afore', copies: 10632 };
  ...
  describe('new Book page', () => {
    ...
    it('should create an instance of Book', () => {
      cy.get(`[data-cy="isbn"]`).type('clearly reach');
      cy.get(`[data-cy="isbn"]`).should('have.value', 'clearly reach');

      cy.get(`[data-cy="name"]`).type('elegantly indeed spring');
      cy.get(`[data-cy="name"]`).should('have.value', 'elegantly indeed spring');

      cy.get(`[data-cy="publishYear"]`).type('slipper rue oily');
      cy.get(`[data-cy="publishYear"]`).should('have.value', 'slipper rue oily');

      cy.get(`[data-cy="copies"]`).type('10852');
      cy.get(`[data-cy="copies"]`).should('have.value', '10852');
